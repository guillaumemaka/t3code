...
interface KiloCodeTurnSnapshot {
  readonly id: TurnId;
  readonly items: Array<unknown>;
}

type KiloCodeSubscribedEvent =
...
interface KiloCodeSessionContext {
  ...
  readonly server: KiloCodeServerConnection;
  readonly directory: string;
  readonly kiloCodeSessionId: string;
  ...
  readonly turns: Array<KiloCodeTurnSnapshot>;
  activeTurnId: TurnId | undefined;
  activeAgent: string | undefined;
  activeVariant: string | undefined;
  /**
   * One-shot guard flipped by `stopKiloCodeContext` / `emitUnexpectedExit`.
   ...
   *   - tears down the KiloCode server process for scope-owned servers.
   */
...
/**
 * Map a tagged KiloCodeRuntimeError produced by {@link runKiloCodeSdk} into
 ...
 * shape by hand.
 */
const toRequestError = (cause: KiloCodeRuntimeError): ProviderAdapterRequestError =>
 ...
 * typed cause is usually an `KiloCodeRuntimeError` (from {@link runKiloCodeSdk}),
 * in which case we preserve its `detail`; otherwise we fall back to
 * {@link kiloCodeRuntimeErrorDetail} for unknown causes (defects, etc.).
 */
const toProcessError = (threadId: ThreadId, cause: unknown): ProviderAdapterProcessError =>
  new ProviderAdapterProcessError({
    provider: PROVIDER,
    threadId,
    detail: KiloCodeRuntimeError.is(cause) ? cause.detail : kiloCodeRuntimeErrorDetail(cause),
    cause,
  });
...
function resolveTurnSnapshot(
  context: KiloCodeSessionContext,
  turnId: TurnId,
): KiloCodeTurnSnapshot {
  const existing = context.turns.find((turn) => turn.id === turnId);
  if (existing) {
    return existing;
  }

  const created: KiloCodeTurnSnapshot = { id: turnId, items: [] };
...
function normalizeQuestionRequest(request: QuestionRequest): ReadonlyArray<UserInputQuestion> {
  return request.questions.map((question, index) => ({
    id: kiloCodeQuestionId(index, question),
...
export function mergeKiloCodeAssistantText(
...
export function appendKiloCodeAssistantTextDelta(
  previousText: string,
  delta: string,
): {
...
function sessionErrorMessage(error: unknown): string {
  if (!error || typeof error !== "object") {
    return "KiloCode session failed.";
  }
...
const stopKiloCodeContext = Effect.fn("stopKiloCodeContext")(function* (
  context: KiloCodeSessionContext,
) {
  ...
  // but we still want to tell KiloCode that this session is done.
  yield* runKiloCodeSdk("session.abort", () =>
    context.client.session.abort({ sessionID: context.kiloCodeSessionId }),
...
export function makeKiloCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // session. Each session's `Scope.close` tears down its spawned KiloCode
    // server (via the `ChildProcessSpawner` finalizer installed in
    // `startKiloCodeServerProcess`) and interrupts the forked event/exit
    // fibers. Consumers that can't reason about Effect scopes therefore
    // cannot leak KiloCode child processes by forgetting to call `stopAll`.
    yield* Effect.addFinalizer(() =>
      Effect.gen(function* () {
        ...
        yield* Effect.forEach(
          contexts,
          (context) => Effect.ignoreCause(stopKiloCodeContext(context)),
    ...
    const emitUnexpectedExit = Effect.fn("emitUnexpectedExit")(function* (
      ...
      // Inline the teardown that `stopKiloCodeContext` would do; we can't
    ...
    const emitAssistantTextDelta = Effect.fn("emitAssistantTextDelta")(function* (
      ...
      const { latestText, deltaToEmit } = mergeKiloCodeAssistantText(previousText, text);
    ...
    const handleSubscribedEvent = Effect.fn("handleSubscribedEvent")(function* (
      ...
      yield* writeNativeEventBestEffort(context.session.threadId, {
        ...
        event: {
          provider: PROVIDER,
          threadId: context.session.threadId,
          providerThreadId: context.kiloCodeSessionId,
          type: event.type,
          ...(turnId ? { turnId } : {}),
          payload: event,
        },
      ...
      switch (event.type) {
        ...
        case "message.part.delta": {
          ...
          const previousText =
            ...
            "";
          const { nextText, deltaToEmit } = appendKiloCodeAssistantTextDelta(previousText, delta);
          if (deltaToEmit.length === 0) {
            break;
          }
        ...
        case "question.replied": {
          ...
          const answers = Object.fromEntries(
            (request?.questions ?? []).map((question, index) => [
              kiloCodeQuestionId(index, question),
        ...
        case "session.turn.close": {
          ...
          if (activeTurnId) {
            ...
            const reason = String(properties.reason ?? properties.status ?? "completed");
        ...
        default:
          yield* Effect.logDebug(
            `Unhandled KiloCode event '${event.type}' for thread ${context.session.threadId}`,
          );
          break;
      }
    });

    const startEventPump = Effect.fn("startEventPump")(function* (context: KiloCodeSessionContext) {
      ...
      yield* Effect.flatMap(
        runKiloCodeSdk("event.subscribe", () =>
          ...
          Effect.gen(function* () {
            ...
            if (Exit.isFailure(exit)) {
              yield* emitUnexpectedExit(
                context,
                kiloCodeRuntimeErrorDetail(Cause.squash(exit.cause)),
              );
            }
          }),
        ),
        ...
        yield* context.server.exitCode.pipe(
          Effect.flatMap((code) =>
            Effect.gen(function* () {
              ...
              yield* emitUnexpectedExit(context, `KiloCode server exited unexpectedly (${code}).`);
            }),
          ),
          Effect.forkIn(context.sessionScope),
        );
    ...
    const startSession: KiloCodeAdapterShape["startSession"] = Effect.fn("startSession")(
      function* (input) {
        const binaryPath = kiloCodeSettings.binaryPath;
        const serverUrl = kiloCodeSettings.serverUrl;
        const serverPassword = kiloCodeSettings.serverPassword;
        const directory = input.cwd ?? serverConfig.cwd;
        const existing = sessions.get(input.threadId);
        if (existing) {
          yield* stopKiloCodeContext(existing);
        ...
        const started = yield* Effect.gen(function* () {
          ...
          const startedExit = yield* Effect.exit(
            Effect.gen(function* () {
              ...
              const server = yield* kiloCodeRuntime.connectToKiloCodeServer({
                binaryPath,
                serverUrl,
              ...
              const kiloCodeSession = yield* runKiloCodeSdk("session.create", () =>
                client.session.create({
                  title: `T3 Code ${input.threadId}`,
                  permission: buildKiloCodePermissionRules(input.runtimeMode),
                }),
              );
              if (!kiloCodeSession.data) {
                return yield* new KiloCodeRuntimeError({
                  operation: "session.create",
                  detail: "KiloCode session.create returned no session payload.",
                });
              }
              return {
                sessionScope,
                server,
                client,
                kiloCodeSession: kiloCodeSession.data,
              };
        ...
        if (raceWinner) {
          ...
          yield* runKiloCodeSdk("session.abort", () =>
            started.client.session.abort({
              sessionID: started.kiloCodeSession.id,
            }),
          ).pipe(Effect.ignore);
        ...
        const context: KiloCodeSessionContext = {
          session,
          client: started.client,
          server: started.server,
          directory,
          kiloCodeSessionId: started.kiloCodeSession.id,
        ...
        yield* emit({
          ...
          payload: {
            message: "KiloCode session started",
          },
        });
        yield* emit({
          ...(yield* buildEventBase({ threadId: input.threadId })),
          type: "thread.started",
          payload: {
            providerThreadId: started.kiloCodeSession.id,
          },
        });
    ...
    const sendTurn: KiloCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
      const context = ensureSessionContext(sessions, input.threadId);
      const turnId = TurnId.make(`kilocode-turn-${yield* Random.nextUUIDv4}`);
      ...
      if (modelSelection !== undefined && modelSelection.instanceId !== boundInstanceId) {
        return yield* new ProviderAdapterValidationError({
          provider: PROVIDER,
          operation: "sendTurn",
          issue: `KiloCode model selection is bound to instance '${modelSelection?.instanceId}', expected '${boundInstanceId}'.`,
        });
      }
      const parsedModel = parseKiloCodeModelSlug(modelSelection?.model);
      if (!parsedModel) {
        return yield* new ProviderAdapterValidationError({
          provider: PROVIDER,
          operation: "sendTurn",
          issue: "KiloCode model selection must use the 'provider/model' format.",
        });
      }
      ...
      if ((!text || text.length === 0) && fileParts.length === 0) {
        return yield* new ProviderAdapterValidationError({
          provider: PROVIDER,
          operation: "sendTurn",
          issue: "KiloCode turns require text input or at least one attachment.",
        });
      }
      ...
      yield* runKiloCodeSdk("session.promptAsync", () =>
        context.client.session.promptAsync({
          sessionID: context.kiloCodeSessionId,
          model: parsedModel,
    ...
    const interruptTurn: KiloCodeAdapterShape["interruptTurn"] = Effect.fn("interruptTurn")(
    ...
    const respondToRequest: KiloCodeAdapterShape["respondToRequest"] = Effect.fn(
      "respondToRequest",
      ...
      yield* runKiloCodeSdk("permission.reply", () =>
        context.client.permission.reply({
          requestID: requestId,
          reply: toKiloCodePermissionReply(decision),
        }),
      ).pipe(Effect.mapError(toRequestError));
    });

    const respondToUserInput: KiloCodeAdapterShape["respondToUserInput"] = Effect.fn(
      "respondToUserInput",
      ...
      yield* runKiloCodeSdk("question.reply", () =>
        context.client.question.reply({
          requestID: requestId,
          answers: toKiloCodeQuestionAnswers(request, answers),
        }),
      ).pipe(Effect.mapError(toRequestError));
    });

    const stopSession: KiloCodeAdapterShape["stopSession"] = Effect.fn("stopSession")(
      function* (threadId) {
        ...
        const stopped = yield* stopKiloCodeContext(context);
    ...
    const readThread: KiloCodeAdapterShape["readThread"] = Effect.fn("readThread")(
      function* (threadId) {
        const context = ensureSessionContext(sessions, threadId);
        const messages = yield* runKiloCodeSdk("session.messages", () =>
    ...
    const rollbackThread: KiloCodeAdapterShape["rollbackThread"] = Effect.fn("rollbackThread")(
      function* (threadId, numTurns) {
        ...
        yield* runKiloCodeSdk("session.revert", () =>
    ...
    const stopAll: KiloCodeAdapterShape["stopAll"] = () =>
      Effect.gen(function* () {
        const contexts = [...sessions.values()];
        sessions.clear();
        // `stopKiloCodeContext` is typed as never-failing — SDK aborts are
