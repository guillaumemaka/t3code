...
import {
  appendKiloCodeAssistantTextDelta,
  makeKiloCodeAdapter,
  mergeKiloCodeAssistantText,
} from "./KiloCodeAdapter.ts";

// Test-local service tag so the rest of the file can keep using `yield* KiloCodeAdapter`.
class KiloCodeAdapter extends Context.Service<KiloCodeAdapter, KiloCodeAdapterShape>()(
  "test/KiloCodeAdapter",
) {}
...
const KiloCodeRuntimeTestDouble: KiloCodeRuntimeShape = {
  startKiloCodeServerProcess: ({ binaryPath }) =>
    Effect.gen(function* () {
  ...
  runKiloCodeCommand: () => Effect.succeed({ stdout: "", stderr: "", code: 0 }),
  createKiloCodeSdkClient: ({ baseUrl, serverPassword }) =>
    ({
      session: {
        create: async () => {
          runtimeMock.state.sessionCreateUrls.push(baseUrl);
          runtimeMock.state.authHeaders.push(
            serverPassword ? `Basic ${btoa(`kilocode:${serverPassword}`)}` : null,
          );
  ...
  loadKiloCodeInventory: () =>
    Effect.fail(
      new KiloCodeRuntimeError({
        ...
        detail: "KiloCodeRuntimeTestDouble.loadKiloCodeInventory not used in this test",
        cause: null,
      }),
    ),
};
...
// decoded `KiloCodeSettings`.
const kiloCodeAdapterTestSettings = Schema.decodeSync(KiloCodeSettings)({
  binaryPath: "fake-kilocode",
  serverUrl: "http://127.0.0.1:9999",
  serverPassword: "secret-password",
});

const KiloCodeAdapterTestLayer = Layer.effect(
  KiloCodeAdapter,
  makeKiloCodeAdapter(kiloCodeAdapterTestSettings),
).pipe(
  Layer.provideMerge(Layer.succeed(KiloCodeRuntime, KiloCodeRuntimeTestDouble)),
...
it.layer(KiloCodeAdapterTestLayer)("KiloCodeAdapterLive", (it) => {
  it.effect("reuses a configured KiloCode server URL instead of spawning a local server", () =>
    Effect.gen(function* () {
      const adapter = yield* KiloCodeAdapter;

      const session = yield* adapter.startSession({
        provider: ProviderDriverKind.make("kilocode"),
        threadId: asThreadId("thread-kilocode"),
        runtimeMode: "full-access",
      });

      assert.equal(session.provider, "kilocode");
      assert.equal(session.threadId, "thread-kilocode");
      ...
      assert.deepEqual(runtimeMock.state.authHeaders, [
        `Basic ${btoa("kilocode:secret-password")}`,
      ]);
    }),
  );
  ...
  it.effect("stops a configured-server session without trying to own server lifecycle", () =>
    Effect.gen(function* () {
      ...
      yield* adapter.stopSession(asThreadId("thread-kilocode"));
  ...
  it.effect("emits one session.exited event when stopping a session", () =>
    Effect.gen(function* () {
      const adapter = yield* KiloCodeAdapter;
      const threadId = asThreadId("thread-kilocode-stop-event");
  ...
  it.effect("clears session state even when cleanup finalizers throw", () =>
    Effect.gen(function* () {
      ...
      // `stopAll` relies on `stopKiloCodeContext`, which is typed as
  ...
  it.effect("completes streamEvents when the adapter scope closes", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        const adapter = yield* Effect.service(KiloCodeAdapter).pipe(Effect.provide(context));
  ...
  it.effect("rolls back session state when sendTurn fails before KiloCode accepts the prompt", () =>
    Effect.gen(function* () {
      const adapter = yield* KiloCodeAdapter;
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("kilocode"),
      ...
      const error = yield* adapter
        .sendTurn({
          ...
          input: "Fix it",
          modelSelection: {
            instanceId: ProviderInstanceId.make("kilocode"),
            model: "openai/gpt-5",
          },
        })
        .pipe(Effect.flip);
      ...
      assert.equal(
        error.message,
        "Provider adapter request failed (kilocode) for session.promptAsync: prompt failed",
      );
  ...
  it.effect("passes agent and variant options for the adapter's bound custom instance id", () => {
    const instanceId = ProviderInstanceId.make("kilocode_zen");
    const adapterLayer = Layer.effect(
      KiloCodeAdapter,
      makeKiloCodeAdapter(kiloCodeAdapterTestSettings, { instanceId }),
    ).pipe(
      Layer.provideMerge(Layer.succeed(KiloCodeRuntime, KiloCodeRuntimeTestDouble)),
      Layer.provideMerge(ServerConfig.layerTest(process.cwd(), process.cwd())),
      Layer.provideMerge(ServerSettingsService.layerTest()),
    ...
    return Effect.gen(function* () {
      ...
      yield* adapter.sendTurn({
        ...
        modelSelection: createModelSelection(
          ProviderInstanceId.make("kilocode_zen"),
  ...
  it.effect("rejects sendTurn model selections for another instance id", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(
        error.issue,
        "KiloCode model selection is bound to instance 'kilocode', expected 'kilocode_zen'.",
      );
  ...
  it.effect("appends raw assistant text deltas and reconciles part update snapshots", () =>
    Effect.sync(() => {
      const firstUpdate = mergeKiloCodeAssistantText(undefined, "Hello");
      const overlapDelta = appendKiloCodeAssistantTextDelta(firstUpdate.latestText, "lo world");
      const secondUpdate = mergeKiloCodeAssistantText(overlapDelta.nextText, "Hellolo world");
  ...
  it.effect("does not strip coincidental prefix overlap from KiloCode part deltas", () =>
    Effect.gen(function* () {
      const adapter = yield* KiloCodeAdapter;
      const threadId = asThreadId("thread-kilocode-raw-delta");
      const part = {
        id: "part-raw-delta",
        sessionID: "http://127.0.0.1:9999/session",
  ...
  it.effect("writes provider-native observability records using the session thread id", () =>
    Effect.gen(function* () {
      ...
      const nativeEventLogger = {
        filePath: "memory://kilocode-native-events",
      ...
      assert.equal(
        nativeEvents.some((record) => record.event?.provider === "kilocode"),
        true,
      );
      assert.equal(
        nativeEvents.some(
